...
void GCApplication::showImage() const
{
	...
	Mat res;
	Mat binMask;
	if( !isInitialized )	//first iter
		image->copyTo( res );
	else
	{
		//not first iter
		getBinMask( mask, binMask );
		image->copyTo( res, binMask );  // 只有前景部分会被复制到res中
	}
...
void GCApplication::mouseClick( int event, int x, int y, int flags, void* )
{
	switch( event )
	{
	case CV_EVENT_LBUTTONDOWN: // Set rectangle or GC_BGD(GC_FGD) labels
		{
			bool isb = (flags & BGD_KEY) != 0,	// enter ctrl
				isf = (flags & FGD_KEY) != 0;	//enter shift
...
int GCApplication::nextIter()
{
	...
	else
	{
		...
		if( lblsState == SET || prLblsState == SET )
		 gc.GrabCut( *image, mask, rect, bgdModel, fgdModel, 10, GC_WITH_MASK );
		else
		 gc.GrabCut(*image, mask, rect, bgdModel, fgdModel,10,GC_WITH_RECT);
		isInitialized = true;
	}
	iterCount++;
